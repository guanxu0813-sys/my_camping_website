...
SEO_OVERRIDES_PATH = DATA / "seo-overrides.json"
...
LOW_VALUE_ACCESSORY_PATTERN = re.compile(
...
def brand_home_path(brand_id: str) -> str:
    return f"/brands/{brand_id}.html"
...
def sorted_products_for_page(page: dict, products: list[dict], brands: dict[str, dict]) -> list[dict]:
    rows = [
        ...
        if not is_low_value_accessory(product)
    ]
...
def is_low_value_accessory(product: dict) -> bool:
    ...
    return product.get("category") in {"tent", "tarp"} and re.search(
        r"\b(dry bag|tent body|tr\d(?:\s+plus)? body)\b",
        model,
        re.IGNORECASE,
    ) is not None


def product_comparison_notes(product: dict, brands: dict[str, dict]) -> str:
    """Keep source context useful without copying an entire storefront description."""
    description = strip_html(product.get("description") or "")
    if not description:
        return product_summary(product, brands)
    first_sentence = re.split(r"(?<=[.!?])\s+", description, maxsplit=1)[0]
    if len(first_sentence) < 90:
        return truncate_text(description, 420)
    return truncate_text(first_sentence, 420)
...
def build_seo_head_block(
    ...
    return (
        ...
        f'  <link rel="alternate" type="application/rss+xml" title="CampGear Compare Guides" href="{site_url}/feed.xml" />\n'
...
def inject_html_seo(
    ...
    nav_pattern = re.compile(
        r'(<ul class="nav__links nav__links--categories">.*?)(\s*</ul>)',
        re.DOTALL,
    )
    nav_match = nav_pattern.search(text)
    if nav_match and 'href="/guides/"' not in nav_match.group(0):
        text = nav_pattern.sub(
            r'\1 <li><a href="/guides/">Guides</a></li>\2',
            text,
            count=1,
        )
...
def static_page_head(
    ...
    robots_meta = (
        f'  <meta name="robots" content="{escape_html(robots)}" />\n'
        if robots
        else ""
    )
    return (
        ...
        f"{robots_meta}"
        '  <link rel="alternate" type="application/rss+xml" title="CampGear Compare Guides" href="/feed.xml" />\n'
...
def similar_weight_competitors(
    ...
    for p in products:
        ...
        if p.get("category") != category or p.get("brandId") == brand_id:
            continue
        if p.get("inSummaryTable") is False or is_low_value_accessory(p):
            continue
...
def product_page_html(
    ...
    title = override.get("title") or product_page_title(product, brands)
    description = override.get("description") or product_meta_description(product, brands)
    ...
    browse_links = []
    for guide_path, guide_title in guide_links or []:
        browse_links.append(
            f'<a href="{escape_html(guide_path)}">{escape_html(guide_title)}</a>'
        )
    ...
    brand_total_count = sum(1 for p in products if p.get("brandId") == brand_id)
    if brand_total_count >= MIN_BRAND_CATEGORY_PRODUCTS:
        browse_links.append(
            f'<a href="{escape_html(brand_home_path(brand_id))}">{escape_html(brand)} gear hub</a>'
        )
    browse_links.append(
    ...
    notes = product_comparison_notes(product, brands)
    current_path = category_page_path(category)
    return (
        "<!DOCTYPE html>\n"
        '<html lang="en">\n'
        f"{static_page_head(title, description, canonical, product_json_ld(product, site_url, brands), image, 'noindex,follow' if noindex else '')}\n"
        ...
        "@graph": [
            {
                ...
                "itemListElement": [
                    ...
                    },
                    {
                        ...
                        "position": 3,
                        "name": brand,
                        "item": f"{site_url}{brand_home_path(brand_id)}",
                    },
                    {
                        "@type": "ListItem",
                        "position": 4,
                        "name": f"{brand} {category_label(category)}",
                        "item": page_url,
                    },
                ],
            },
...
def brand_page_html(
    ...
    title = override.get("title") or (
        f"{brand} {label} Comparison — {len(rows)} Models by Weight & Price"
    )
    description = override.get("description") or truncate_text(
    ...
    return (
        ...
        f'    <span aria-hidden="true">/</span><a href="{escape_html(brand_home_path(brand_id))}">{escape_html(brand)}</a>\n'
        f'    <span aria-hidden="true">/</span><span>{escape_html(label)}</span>\n'
        "  </nav>\n"
        "  <header class=\"page__head\">\n"
...
def brand_home_page_html(
    ...
    grouped: dict[str, list[dict]] = defaultdict(list)
    for product in rows:
        grouped[product.get("category", "other")].append(product)
    title = f"{brand} Camping Gear — {len(rows)} Models by Category"
    description = truncate_text(
        f"Explore {len(rows)} {brand} camping gear models across "
        f"{len(grouped)} categories. Compare official-source weight, capacity and price data."
    )
    canonical = f"{site_url}{brand_home_path(brand_id)}"
    sections = []
    item_list = []
    for position, (category, category_rows) in enumerate(
        sorted(grouped.items(), key=lambda item: category_label(item[0])),
        start=1,
    ):
        category_href = (
            brand_category_path(brand_id, category)
            if len(category_rows) >= MIN_BRAND_CATEGORY_PRODUCTS
            else category_page_path(category)
        )
        product_links = "".join(
            f'<li><a href="{escape_html(product_path(product))}">'
            f"{escape_html(product.get('model', ''))}</a></li>"
            for product in sorted(
                category_rows,
                key=lambda product: (product.get("model") or "").lower(),
            )[:8]
        )
        sections.append(
            '<section class="static-panel">'
            f'<h2><a href="{escape_html(category_href)}">{escape_html(category_label(category))}</a> '
            f"<span>({len(category_rows)})</span></h2>"
    ...
    return (
        "<!DOCTYPE html>\n"
        '<html lang="en">\n'
        f"{static_page_head(title, description, canonical, schema)}\n"
        '<body data-page="brand">\n'
        f"{static_header()}\n"
        '<main class="page page--sub static-page">\n'
        '  <nav class="static-breadcrumb" aria-label="Breadcrumb">\n'
        '    <a href="/index.html">Home</a><span aria-hidden="true">/</span>\n'
        f"    <span>{escape_html(brand)}</span>\n"
        "  </nav>\n"
        '  <header class="page__head">\n'
        '    <p class="page__eyebrow">Brand gear hub</p>\n'
...
def sorted_visible_products(products: list[dict], brands: dict[str, dict]) -> list[dict]:
    ...
    return sorted(
        (product for product in products if not is_low_value_accessory(product)),
        key=sort_key,
    )
...
def write_static_pages(
    ...
    all_rows = sorted(
        products,
        key=lambda product: (
            brand_rank(brands.get(product.get("brandId", ""))),
            product.get("category", ""),
            (product.get("model") or "").lower(),
        ),
    )
    rows = sorted_visible_products(products, brands)
    accessory_rows = [
        product for product in all_rows if is_low_value_accessory(product)
    ]
    affiliates = load_affiliates()
    affiliate_links = load_affiliate_links()
    stats = build_category_stats(rows)
    overrides = seo_overrides or {}
    product_overrides = overrides.get("products") or {}
    brand_overrides = overrides.get("brands") or {}
    for product in rows:
        ...
        path.write_text(
            product_page_html(
                ...
                affiliate_links,
                stats,
                product_overrides.get(product.get("id", "")),
                (guide_links or {}).get(product.get("id", ""), []),
            ),
            encoding="utf-8",
        )
        entries.append(
    ...
    for product in rows:
        ...
        category = product.get("category")
        if brand_id and category:
            brand_groups[brand_id].append(product)
            grouped[(brand_id, category)].append(product)

    for brand_id, brand_rows in sorted(
        brand_groups.items(), key=lambda item: brand_rank(brands.get(item[0]))
    ):
        if len(brand_rows) < MIN_BRAND_CATEGORY_PRODUCTS:
            continue
        ...
        entries.append(
            {
                "loc": f"{site_url}{brand_home_path(brand_id)}",
                "lastmod": max(
                    source_lastmod_for_product(product) for product in brand_rows
                ),
                "changefreq": "weekly",
                "priority": "0.7",
                "group": "brands",
...
def main() -> int:
    ...
    print(
        ...
        f"{sum(1 for entry in generated_entries if entry.get('group') == 'products')} "
        "indexable product page(s)"
    )
    return 0
