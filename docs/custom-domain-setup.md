...
> **主域名（canonical）：** `https://www.campgearcompare.com`  
...
- **Canonical：** `https://www.campgearcompare.com`（无 www）
...
```bash
# 根域可访问
curl -sI https://www.campgearcompare.com | head -3

# www 重定向到根域
curl -sI https://www.campgearcompare.com | grep -i location

# sitemap / robots
curl -s https://www.campgearcompare.com/sitemap.xml | head -5
curl -s https://www.campgearcompare.com/robots.txt
...
2. 选 **网址前缀**：`https://www.campgearcompare.com`
3. 验证方式（任选其一）：
   - **HTML 文件**：下载验证文件 → 放到项目根目录 → push 部署
   - **DNS TXT 记录**：在域名 DNS 添加 Google 提供的 TXT
4. 提交 sitemap：`https://www.campgearcompare.com/sitemap.xml`
...
- 对外链接统一改为 `https://www.campgearcompare.com`
